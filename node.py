...
class ServerThread(Thread):
    ...
    def _update(self):
        ...
        self._serverSocket = \
            utils.CreateServerSocket(config.NODE_PORT[self._node.NodeID])
        self._connectionList.append(self._serverSocket)
        while True:
            (read_sockets, write_sockets, error_sockets) = \
                select.select(self._connectionList, [], [])
    ...
    def _onRequest(self, request_msg):
        if self._node.State == STATE.HELD:
            ...
            if self._node.HasVoted:
                ...
                if request_msg < self._node.VotedRequest \
                    and not self._node.HasInquired:
                ...
                self._node.Client.SendMessage(response_msg,
                        response_msg.dest)
            else:
    ...
    def _grantRequest(self, request_msg):
        grant_msg = Message(msg_type=MSG_TYPE.GRANT,
                            src=self._node.NodeID, dest=request_msg.src)
    ...
    def _onYield(self, yield_msg):
        heapq.heappush(self._node.RequestQueue, self._node.VotedRequest)
        self._onRelease()
...
class ClientThread(Thread):
    ...
    def __init__(self, node):
        Thread.__init__(self)
        self._node = node
        self._clientSockets = [utils.CreateClientSocket() for i in
...
............))'''
    ...
    def BuildConnection(self, num_node):
        for i in xrange(num_node):
            self._clientSockets[i].connect(('localhost',
                    config.NODE_PORT[i]))
    ...
    def _update(self):
        while True:
            ...
            for i in xrange(config.NUM_NODE):
                while self._node.MessageBuffer[i] \
                    and self._node.MessageBuffer[i][0][1] <= curr_time:
...
class Node(object):
    ...
    def _createVotingSet(self):
        ...
        (row_id, col_id) = (int(self.NodeID / mat_k), int(self.NodeID
                            % mat_k))
    ...
    def EnterCS(self, ts):
        ...
        self.LamportTS += 1
        sys.stdout.write('{src}: enter CS at {t}\n'.format(src=self.NodeID,
                         t=utils.DatetimeToStr(ts)))
        self.SignalEnterCS.clear()

    def ExitCS(self, ts):
        self.TimeRequestCS = ts \
            + datetime.timedelta(milliseconds=NEXT_REQ)
    ...
    def _check(self):
        ...
        curr_time = datetime.datetime.now()
        if self.State == STATE.RELEASE and self.TimeRequestCS \
            <= curr_time:
            if not self.SignalRequestCS.is_set():
                self.SignalRequestCS.set()
        elif self.State == STATE.REQUEST and self.NumVotesReceived \
            == len(self.VotingSet):
